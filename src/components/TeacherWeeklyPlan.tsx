      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 sm:mb-8 gap-3 sm:gap-4">
        <div>
          <div className="flex flex-wrap items-center gap-1.5 sm:gap-3 mb-2">
            <span className="text-xs sm:text-sm text-emerald-700 bg-emerald-50 px-2.5 sm:px-3 py-1 rounded-full font-medium border border-emerald-100">Năm học {schoolYearName || '2024 - 2025'} • Lớp {className || 'Chưa chọn lớp'}</span>
            <span className="text-xs sm:text-sm text-indigo-700 bg-indigo-50 px-2.5 sm:px-3 py-1 rounded-full font-medium border border-indigo-100">Đã duyệt: {weeks.filter(w => w.status === 'approved').length} / 42 tuần</span>
          </div>
          ...
          <p className="text-slate-500 text-xs sm:text-sm mt-0.5 sm:mt-1">Biên soạn công tác nề nếp, phân công tổ trực nhật và theo dõi phê duyệt của Ban Giám Hiệu</p>
        </div>
        <div className="flex flex-col sm:flex-row gap-2 w-full md:w-auto">
          <button 
            ...
            className="flex items-center justify-center gap-2 px-3 sm:px-4 py-2 text-indigo-600 bg-indigo-50 font-medium rounded-xl hover:bg-indigo-100 transition-colors text-xs sm:text-sm shadow-2xs">
          ...
          <button 
            ...
            className="flex items-center justify-center gap-2 px-3 sm:px-4 py-2 bg-teal-600 text-white font-medium rounded-xl hover:bg-teal-700 transition-colors text-xs sm:text-sm shadow-2xs">
